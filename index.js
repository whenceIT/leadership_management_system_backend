...
// =======================================================
// 1) STAFF ADEQUACY (company avg of branch scores)
// =======================================================
app.get("/staff-adequacy/company", async (req, res) => {
  try {
    const offices = await getAllOffices(pool);
    ...
    for (const office of offices) {
      ...
      const ActualLCs = rows[0]?.total || 0;
    ...
    const average_normalized_score = avg(scores);
    const percentage_point = average_normalized_score * 0.25;

    res.json({
      offices_count: scores.length,
      average_normalized_score: average_normalized_score.toFixed(2),
      weight: "25%",
...
// =======================================================
// 2) PRODUCTIVITY ACHIEVEMENT (company avg of branch scores)
// =======================================================
app.get("/productivity-achievement/company", async (req, res) => {
  try {
    const offices = await getAllOffices(pool);
    ...
    for (const office of offices) {
      ...
      for (const user of consultants) {
        const cycleEndDay = user.cycle_end_date ? parseInt(user.cycle_end_date) : 24;
        ...
        const [loans] = await pool.query(`SELECT id FROM loans WHERE loan_officer_id = ?`, [
          user.id,
        ]);
        ...
        transactions.forEach((t) => {
          ...
          if (t.transaction_type === "repayment" && t.payment_apply_to === "reloan_payment") {
            const reloan_amount = (Number(t.balance_bf) || 0) - (Number(t.credit) || 0);
        ...
        const carryOverAmount = carryOver.length ? Number(carryOver[0].amount) : 0;
      ...
      const average_disbursement =
        consultant_count > 0 ? branch_total_given_out / consultant_count : 0;

      let normalized_score = (average_disbursement / 40000) * 100;
      if (normalized_score < 0) normalized_score = 0;
    ...
    const average_normalized_score = avg(scores);
    const percentage_point = average_normalized_score * 0.30;
...
// =======================================================
// 3) VACANCY IMPACT (company avg of branch scores)
// =======================================================
app.get("/vacancy-impact/company", async (req, res) => {
  try {
    ...
    const average_normalized_score = avg(scores);
    const percentage_point = average_normalized_score * 0.20;

    res.json({
      offices_count: scores.length,
      authorized_positions_per_office: AuthorizedPositions,
      average_normalized_score: Number(average_normalized_score.toFixed(4)),
      weight: "20%",
      percentage_point: Number(percentage_point.toFixed(4)),
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server error" });
...
// =======================================================
// 4) PORTFOLIO LOAD BALANCE (company avg of branch scores)
// =======================================================
app.get("/portfolio-load-balance/company", async (req, res) => {
  try {
    const offices = await getAllOffices(pool);
    ...
    for (const office of offices) {
      ...
      for (let loan of loans) {
        ...
        const outstanding = (Number(loan.principal) || 0) - total_repaid;

        if (outstanding > 0) total_outstanding += outstanding;
      }
      ...
      if (portfolio_per_lc < lowerBound) {
        const deviationPercent = Math.abs(portfolio_per_lc - lowerBound) / lowerBound;
        score = 100 - deviationPercent * 50;
      }

      if (portfolio_per_lc > upperBound) {
        const deviationPercent = Math.abs(portfolio_per_lc - upperBound) / upperBound;
        score = 100 - deviationPercent * 50;
      }
    ...
    const average_score = avg(scores);
    const percentage_point = average_score * 0.25;
...
// =======================================================
// 5) VOLUME ACHIEVEMENT (company avg of branch scores)
// =======================================================
app.get("/volume-achievement/company", async (req, res) => {
  try {
    const offices = await getAllOffices(pool);
...
// =======================================================
// 6) PORTFOLIO QUALITY (company avg of branch scores)
// =======================================================
app.get("/portfolio-quality/company", async (req, res) => {
  try {
    const offices = await getAllOffices(pool);
    ...
    for (const office of offices) {
      ...
      for (let loan of loans) {
        ...
        if (outstanding > 0) {
          ...
          if (diffDays > 30) overdue_outstanding += outstanding;
        }
      }
    ...
    const average_score = avg(scores);
    const percentage_point = average_score * 0.35;
...
// =======================================================
// 7) COLLECTION EFFICIENCY (company avg of branch scores)
// =======================================================
app.get("/collection-efficiency/company", async (req, res) => {
  try {
    ...
    for (const office of offices) {
      ...
      for (const user of consultants) {
        ...
        const [loans] = await pool.query(`SELECT id, principal FROM loans WHERE loan_officer_id = ?`, [
          user.id,
        ]);
      ...
      const collections_rate =
        total_outstanding > 0 ? (branch_total_collections / total_outstanding) * 100 : 0;
    ...
    const average_score = avg(scores);
    const percentage_point = average_score * 0.30;
...
app.get('/vetting-compliance-rate/company', async (req, res) => {
  try {
        ...
        SELECT 
          ...
          COUNT(DISTINCT CASE
            WHEN ci.client_id IS NOT NULL
              AND c.picture IS NOT NULL
              AND c.picture <> ''
              AND d.record_id IS NOT NULL
              AND nk.client_id IS NOT NULL
              AND l.vetted_by IS NOT NULL
    ...
    const offices_count = rows.length;
    const average_score =
      rows.reduce((sum, row) => sum + (Number(row.office_score) || 0), 0) / offices_count;
...
// =======================================================
app.get("/yield-achievement/company", async (req, res) => {
  try {
    const TARGET = 0.382; // 38.2%
    const offices = await getAllOffices(pool);
    ...
    for (const office of offices) {
      ...
      const total_interest_earned = total_collected - total_principal_disbursed;

      const effective_interest_rate =
        total_principal_disbursed > 0 ? total_interest_earned / total_principal_disbursed : 0;

      let score = TARGET > 0 ? (effective_interest_rate / TARGET) * 100 : 0;
      if (score > 100) score = 100;
      if (score < 0) score = 0;
...
// =======================================================
// 9) PRODUCT DIVERSIFICATION (HHI) (company avg of branch HHIs)
// =======================================================
app.get("/product-diversification/company", async (req, res) => {
  try {
    const offices = await getAllOffices(pool);
    ...
    for (const office of offices) {
      ...
      const counts = {};
      clients.forEach((c) => {
        const place = c.working_place || "Unknown";
    ...
    const average_HHI = avg(hhis);
    const percentage_point = average_HHI * 0.25;

    return res.json({
      offices_count: hhis.length,
      average_HHI: average_HHI.toFixed(4),
      weight: "25%",
...
// =======================================================
// 10) PRODUCT RISK SCORE (company avg of branch score ratios)
// NOTE: keeps your ratio-style score (defaulted_rate / 0.2836)
// =======================================================
app.get("/product-risk-score/company", async (req, res) => {
  try {
    const TARGET_DEFAULT_RATE = 0.2836;
    const today = dayjs();
    ...
    for (const office of offices) {
      ...
      for (const user of consultants) {
        ...
        transactions.forEach((t) => {
          ...
          if (t.transaction_type === "repayment" && t.payment_apply_to === "full_payment") {
      ...
      const score = TARGET_DEFAULT_RATE > 0 ? defaulted_rate / TARGET_DEFAULT_RATE : 0;
    ...
    return res.json({
      offices_count: scores.length,
      average_score: average_score.toFixed(4), // ratio
      weight: "30%",
      percentage_point: percentage_point.toFixed(4),
    });
  } catch (error) {
...
// =======================================================
// 11) MONTH-1 DEFAULT PERFORMANCE (company avg of branch scores)
// =======================================================
app.get("/month-1-default-performance/company", async (req, res) => {
  try {
    const today = dayjs();
    const offices = await getAllOffices(pool);
    ...
    const average_score = avg(scores);
    const percentage_point = average_score * 0.40;
...
// =======================================================
// 12) 3-MONTH RECOVERY ACHIEVEMENT (company avg of branch scores)
// =======================================================
app.get("/3-month-recovery-achievement/company", async (req, res) => {
  try {
    ...
    for (const office of offices) {
      ...
      const recovery_rate_3_months =
        month_1_defaulted > 0 ? recovered_3_months / month_1_defaulted : 0;
...
// =======================================================
// 13) ROLL RATE CONTROL (company avg of branch scores)
// =======================================================
app.get("/roll-rate-control/company", async (req, res) => {
  try {
    const offices = await getAllOffices(pool);
    ...
    for (const office of offices) {
      ...
      for (let loan of loans) {
        ...
        if (diffDays > 0 && diffDays <= 30) bucket_1_30++;
        else if (diffDays > 30 && diffDays <= 60) bucket_31_60++;
        else if (diffDays > 60 && diffDays <= 90) bucket_61_90++;
        else if (diffDays > 90) bucket_90_plus++;
      }
      ...
      const score = ((rr_1_30 + rr_31_60 + rr_61_90 + rr_90_plus) / 4) * 100;
      scores.push(score);
    }

    const average_score = avg(scores);
    const percentage_point = average_score * 0.20;
...
// =======================================================
// 14) LONG TERM DELINQUENCY RISK (company avg of branch scores)
// =======================================================
app.get("/long-term-delinquency-risk/company", async (req, res) => {
  try {
    ...
    for (const office of offices) {
      ...
      const long_term_default_rate =
        month_1_default_loans > 0 ? long_term_delinquent_loans / month_1_default_loans : 0;
...
// =======================================================
// 15) REVENUE ACHIEVEMENT (company avg of branch scores)
// =======================================================
app.get("/revenue-achievement/company", async (req, res) => {
  try {
    ...
    for (const office of offices) {
      ...
      let score = EXPECTED_REVENUE > 0 ? (total_revenue / EXPECTED_REVENUE) * 100 : 0;
      if (score > 100) score = 100;
      if (score < 0) score = 0;
...
// =======================================================
// 16) EFFICIENCY RATIO (company avg of branch scores)
// NOTE: kept your exact scoring logic (0.55 / CIR) and 100 cap.
// =======================================================
app.get("/efficiency-ratio/company", async (req, res) => {
  try {
    const start_date = dayjs().startOf("month").format("YYYY-MM-DD");
    const end_date = dayjs().endOf("month").format("YYYY-MM-DD");
    ...
    for (const office of offices) {
      ...
      const operating_costs = expenses.reduce((sum, e) => sum + (Number(e.amount) || 0), 0);
      ...
      for (const user of consultants) {
        ...
        transactions.forEach((t) => {
          // disbursement in previous month
          if (
      ...
      const CIR = income > 0 ? operating_costs / income : 0;

      let score = CIR > 0 ? 0.55 / CIR : 0;
      if (score > 100) score = 100;
      if (score < 0) score = 0;
...
// =======================================================
// 17) PROFITABILITY CONTRIBUTION (company avg of branch scores)
// NOTE: This is still "avg of branch ratios" (as requested).
// =======================================================
app.get("/profitability-contribution/company", async (req, res) => {
  try {
    const start_date = dayjs().startOf("month").format("YYYY-MM-DD");
    const end_date = dayjs().endOf("month").format("YYYY-MM-DD");

    // 1) company operating costs
    ...
    const company_revenue = companyRevenueTx.reduce((sum, t) => sum + (Number(t.credit) || 0), 0);

    const company_net_contribution = company_revenue - company_operating_costs;
    ...
    for (const office of offices) {
      ...
      const branch_revenue = branchRevenueTx.reduce((sum, t) => sum + (Number(t.credit) || 0), 0);

      const branch_net_contribution = branch_revenue - branch_operating_costs;

      let score = company_net_contribution !== 0 ? branch_net_contribution / company_net_contribution : 0;
      if (score > 1.5) score = 1.5;
      if (score < 0) score = 0;

      scoresPct.push(score * 100); // store percent up to 150%
    }

    const average_score_pct = avg(scoresPct);
    const avg_ratio = average_score_pct / 100;
    const percentage_point = avg_ratio * 0.20;

    return res.json({
      offices_count: scoresPct.length,
      period: `${start_date} to ${end_date}`,
      company_net_contribution: company_net_contribution.toFixed(2),
      average_score: average_score_pct.toFixed(2) + "%",
      weight: "20%",
      percentage_point: percentage_point.toFixed(2),
    });
  } catch (error) {
...
// =======================================================
// 18) GROWTH TRAJECTORY (company avg of branch scores)
// =======================================================
app.get("/growth-trajectory/company", async (req, res) => {
  try {
    // current month
    ...
    for (const office of offices) {
      ...
      scores.push(score);
      moms.push(momRevenue);
    }

    const average_score = avg(scores);
    const avg_mom_revenue = avg(moms);
    const PP = average_score;
...
// Staff Adequacy - District Average
app.get("/staff-adequacy/district/:district_id", async (req, res) => {
  try {
    ...
    rows.forEach(row => {
      ...
      DistrictScores.push(normalized_score);
    });

    const district_average =
      DistrictScores.reduce((sum, score) => sum + score, 0) /
      (DistrictScores.length || 1);

    const PercentagePoint = district_average * 0.25;
...
app.get('/productivity-achievement/district/:district_id', async (req, res) => {
  try {
    ...
    for (const office of offices) {
      ...
      district_scores.push(normalized_score);
    ...
    const district_average =
      district_scores.reduce((sum, score) => sum + score, 0) /
      (district_scores.length || 1);

    const percentage_point = district_average * 0.30;

    res.json({
      district_id,
      offices_count: district_scores.length,
      average_normalized_score: district_average.toFixed(2),
      weight: "30%",
      percentage_point: percentage_point.toFixed(2)
    });
...
app.get("/vacancy-impact/district/:district_id", async (req, res) => {
  try {
    const { district_id } = req.params;
    ...
    for (const office of offices) {
      ...
      district_scores.push(NormalizedScore);
    }
    ...
    const PercentagePoint = district_average * 0.20;
...
//District
app.get('/volume-achievement/district/:district_id', async (req, res) => {
  try {
    ...
    const percentage_point = district_average * 0.25;
...
//district
app.get('/portfolio-quality/district/:district_id', async (req, res) => {
  try {
    ...
    const percentage_point = district_average * 0.35;

    res.json({
      district_id,
      offices_count: district_scores.length,
      average_score: district_average.toFixed(2) + "%",
      weight: "35%",
      percentage_point: percentage_point.toFixed(2)
    });
...
//district
app.get('/vetting-compliance-rate/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
...
app.get('/collection-efficiency/district/:district_id', async (req, res) => {
  try {
...
//district
app.get('/yield-achievement/district/:district_id', async (req, res) => {
  try {
    ...
    for (const office of offices) {
      ...
      district_scores.push(capped_score);
    }
    ...
    // ===============================
    const district_average =
      district_scores.reduce((sum, s) => sum + s, 0) /
      (district_scores.length || 1);
...
//district
app.get('/product-diversification/district/:district_id', async (req, res) => {
  try {
    ...
    for (const office of offices) {
      ...
      district_hhis.push(hhi);
    }

    const avg_hhi =
      district_hhis.reduce((sum, h) => sum + h, 0) /
      (district_hhis.length || 1);

    const percentage_point = avg_hhi * 0.25;

    return res.json({
      district_id,
      offices_count: district_hhis.length,
      average_HHI: avg_hhi.toFixed(4),
      weight: "25%",
      percentage_point: percentage_point.toFixed(4)
    });
...
//district
app.get('/product-risk-score/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
    ...
    for (const office of offices) {
      ...
      district_scores.push(score_ratio);
    }
    ...
    return res.json({
      district_id,
      offices_count: district_scores.length,
      average_score: avg_score_ratio.toFixed(4),
      weight: "30%",
      percentage_point: percentage_point.toFixed(4)
    });
...
//district
app.get('/month-1-default-performance/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
    ...
    const percentage_point = district_average * 0.40;
...
//district
app.get('/3-month-recovery-achievement/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
...
//district
app.get('/roll-rate-control/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
    ...
    const percentage_point = district_average * 0.20;
...
//district
app.get('/long-term-delinquency-risk/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
    ...
    const percentage_point = district_average * 0.10;
...
//district
app.get('/revenue-achievement/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
...
//district
app.get('/efficiency-ratio/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
...
//district
app.get('/profitability-contribution/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
    ...
    for (const office of offices) {
      ...
      district_scores.push(score * 100);
    }
    ...
    const avg_ratio = district_average / 100;
...
//district
app.get('/growth-trajectory/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
...
app.get('/cash-position-score/:office_id', async (req, res) => {
  try {
    ...
    // PART PAYMENTS
    // ===============================
    const [[partPayment]] = await pool.query(`
      SELECT SUM(credit) total
      FROM loan_transactions
      WHERE office_id = ?
      AND payment_apply_to = 'part_payment'
    ...
    const partPaymentTotal = Number(partPayment.total) || 0;
...
app.get('/above-threshold-risk/:office_id', async (req, res) => {
  try {
    ...
    if (closingBalance <= 30000) {
      ...
      const unapproved_excess = closingBalance - 30000;
      score = 100 * (1 - (unapproved_excess / closingBalance));
    }
...
app.get('/below-threshold-risk/:office_id', async (req, res) => {
  try {
    ...
    if (closingBalance >= 20000) {
      ...
      score = (closingBalance / 20000) * 100;
    }
...
app.get('/cash-position-score/province/:province_id', async (req, res) => {
  try {
    const { province_id } = req.params;
...
//district
app.get('/cash-position-score/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
...
app.get('/above-threshold-risk/province/:province_id', async (req, res) => {
  try {
    const { province_id } = req.params;
...
//district
app.get('/above-threshold-risk/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
...
app.get('/above-threshold-risk/company', async (req, res) => {
  try {
    const [offices] = await pool.query(`
      SELECT id
      FROM offices
...
app.get('/below-threshold-risk/province/:province_id', async (req, res) => {
  try {
    const { province_id } = req.params;
...
//district
app.get('/below-threshold-risk/district/:district_id', async (req, res) => {
  try {
    const { district_id } = req.params;
...
app.get('/below-threshold-risk/company', async (req, res) => {
  try {
    const [offices] = await pool.query(`
      SELECT id
      FROM offices
